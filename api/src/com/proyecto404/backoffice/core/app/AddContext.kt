...
data class AddContext(private val context: String): PureCommand {
    internal class Handler(private val repositories: RepositoryProvider): RequestHandler<AddContext, Unit> {

        override fun execute(request: AddContext, identity: Identity) {
            val chatbot = repositories.chatbotConfigurations().get()
            chatbot.addContext(request.context)
